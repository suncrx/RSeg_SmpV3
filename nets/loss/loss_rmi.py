# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 15:22:01 2024

https://github.com/RElbers/region-mutual-information-pytorch
...
class RMILoss(_Loss):    
    ...
    def forward(self, pred, target):
        ...
        if self.bce_weight != 0:
            if self.with_logits:
                bce = F.binary_cross_entropy_with_logits(pred, target=target)
            else:
                bce = F.binary_cross_entropy(pred, target=target)
    ...
    def rmi_loss(self, pred, target):
        ...
        p = self.extract_region_vector(pred)
...
if __name__ == '__main__':
    pred = (torch.rand(8,1,50,70)>0.5).to(dtype=torch.float32)
    targ = (torch.rand(8,1,50,70)>0.5).to(dtype=torch.float32)
    #print(pred)
    #print(targ)
